...
final forecastServiceProvider =
    Provider<ForecastService>((_) => const ForecastService());

final mentorServiceProvider = Provider<MentorService>((ref) {
  return MentorService(ref.watch(aiServiceProvider));
});

final financeForecastProvider =
    Provider<AsyncValue<FinanceForecast>>((ref) {
  ...
  return AsyncValue.data(ref.watch(forecastServiceProvider).buildFinance(
        txs: txA.value ?? const [],
...
final gymForecastProvider = Provider<AsyncValue<GymForecast>>((ref) {
  ...
  final xs = <AsyncValue<Object?>>[sessA, entA, valA, typA];
  for (final a in xs) {
    if (a.isLoading && !a.hasValue) return const AsyncValue.loading();
  }
  for (final a in xs) {
  ...
  return AsyncValue.data(ref.watch(forecastServiceProvider).buildGym(
...
final productivityForecastProvider =
    Provider<AsyncValue<ProductivityForecast>>((ref) {
  final now = DateTime.now();
  ...
  return AsyncValue.data(ref.watch(forecastServiceProvider).buildProductivity(
        tasks: tA.value ?? const [],
        now: now,
      ));
});
